...
<font color='#FF0000' face='Arial Black' size='4'>{$LAN_29}</font></div>
{/if}
{/section}
...
<p>
{if $video[video].approved eq "1"}<font color='#00CC00' face='Arial Black' size='4'>{$LAN_52}d</font>{else}<font color='#FF0000' face='Arial Black' size='4'>Un{$LAN_52}d</font>{/if}
{if $video[video].feature eq "1"}<font color='#00CC00' face='Arial Black' size='4'>-{$LAN_53}ed</font>{/if}
{if $video[video].reject eq "1"}<font color='#FF0000' face='Arial Black' size='4'>-{$LAN_54}ed</font>{/if}
<br />
<img border='0' src='{if $video[video].video_type eq "YouTube"}http://img.youtube.com/vi/{$video[video].file}/1.jpg' height='100'>
<img border='0' src='http://img.youtube.com/vi/{$video[video].file}/2.jpg' height='100'>
<img border='0' src='http://img.youtube.com/vi/{$video[video].file}/3.jpg' height='100'>
{else}
...
{if $video[video].reject eq "0"}
<a href='?{if $pt eq "approve"}id={$video[video].id}&amp;{/if}what=reject&amp;pag=vid&amp;pt={$pt}&amp;id={$id}'>{$LAN_54}</a> or
{/if}

<a href='?{if $pt eq "approve"}id={$video[video].id}&amp;{/if}what=delete&amp;pag=vid&amp;pt={$pt}&amp;id={$id}'>{$LAN_56}</a></div>
...
<br />
<b>{$LAN_35}:</b>{$video[video].description}
<br /><br />
