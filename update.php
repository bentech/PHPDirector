...
require("db.php");
	...
	if($row1["version"] == $version_need){
	echo'Updated<br />';
	mysql_query("UPDATE `pp_config` SET `version` = '0.3 beta'");
	
	mysql_query('ALTER TABLE `pp_files` CHANGE `videoid` `picture` VARCHAR(254) CHARACTER SET latin1 COLLATE latin1_swedish_ci NOT NULL');
	mysql_query('ALTER TABLE `pp_files` CHANGE `picture` `picture` VARCHAR(254) CHARACTER SET latin1 COLLATE latin1_swedish_ci NOT NULL');

	

	}else{
	if ($row1["version"] == $to
	echo "You need ".$version_need;
	}
?>
<br /><br />
